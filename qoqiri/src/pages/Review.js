...
import { saveReview, updateReview, deleteReview, updatePostTitleDropbox } from '../api/review';
...
const ReviewBoard = () => {
    ...
    const fetchUserPosts = async (userId) => {
        try {
            ...
            const filteredPosts = response.data.filter((post) => post.postTitleDropbox !== 'Y');
    ...
    const handleWriteClick = async () => {
        ...
        if (content.length <= 50) {
            ...
            try {
                ...
                const selectedPost = userPosts.find((post) => post.postTitle === selectedTitle);
                window.location.replace('http://localhost:3000/review');
                if (selectedPost) {
                ...
                setContent('');
                setUserPosts((prevPosts) => prevPosts.filter((post) => post.postTitle !== selectedTitle)); // 드롭박스 만들기 및 선택하기
    ...
    const handleWriterClick = (userId) => {
        let modalUrl = loggedInUser && userId === loggedInUser.id ? `/mini/${userId}` : `/mini/${userId}`;
    ...
    return (
        <div className="rv-container">
            ...
            <div className="rv-input-area">
                <select className="rv-select" value={selectedTitle} onChange={(e) => handleTitleSelect(e.target.value)}>
                        ...
                        <div key={po.postSEQ} className="rv-review-item">
                            {isEditing && editingPostId === po.postSEQ ? (
                                <>
                                    ...
                                    ></textarea>
                                    <button className="rv-psUpdataOKBtn" onClick={handleUpdateSubmit}>
                                        수정 완료
                                    </button>
                                </>
                            ) : (
                                <>
                                    <div className="rv-writerInfo">
                                        ...
                                        <span className="rv-writer-title">글 제목 : {po.postTitle}</span>
                                    </div>
                                    {loggedInUser.id === po.userInfo.userId && (
                                        <div className="rv-persnalBtn">
                                            <button
                                                ...
                                                onClick={() =>
                                                    handleEditClick(po.postSEQ, po.postContent, po.postTitle)
                                                }
                                            >
                                                수정
                                            </button>
                                            <button
                                                ...
                                                onClick={() =>
                                                    handleDeleteClick(po.postSEQ, po.userInfo.userId, po.postTitle)
                                                }
                                            >
                                                삭제
                                            </button>
